// Function will run as soon as the page loads
$(function() {
  ...
  $('.movie-button').on('click',function(){
    var my_api_key = '1d1308de31aff9aeb11839d656f9a715';
    var title = $('.movie').val();
    if (title=="") {
...
// get the basic details of the movie from the TMDB-API (based on the name of the movie)
const load_details = (my_api_key, title) => {
  $.ajax({
    type: 'GET',
...
// passing the movie name to get the similar movies from flask server at /similarity
const movie_recs = (movie_title,movie_id,my_api_key) => {
  $.ajax({
    type:'POST',
    url:"/similarity",
...
const get_movie_details = (movie_id,my_api_key,arr,movie_title) => {
  $.ajax({
    type:'GET',
...
// passing all the details to flask server at /recommend for displaying and scraping the movie reviews using imdb id
const show_details = (movie_details,arr,movie_title,my_api_key,movie_id) => {
  ...
  arr_poster = get_movie_posters(arr,my_api_key); // MOVIE POSTER(IMAGE)
  
  movie_cast = get_movie_cast(movie_id,my_api_key); // CAST ID, CAST NAME, CHARACTER NAME, CAST PROFILE
  
  ind_cast = get_individual_cast(movie_cast,my_api_key); //DOB, BIO, PLACE OF BIRTH
...
const get_individual_cast = (movie_cast,my_api_key) => {
    cast_bdays = [];
    cast_bios = [];
    cast_places = [];
...
const get_movie_cast = (movie_id,my_api_key) => {
    cast_ids= [];
    cast_names = [];
    cast_chars = [];
    cast_profiles = [];
...
const get_movie_posters = (arr,my_api_key) => {
  var arr_poster_list = []
  for(var m in arr) {
    $.ajax({
      type:'GET',
